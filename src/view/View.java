...
public class View {
    ...
    public void printSupply(Supply supply) {
        ...
        for (SupplyPile pile : supply.getCardList()) {
            System.out.printf("%2s) %-10s $%s%n", num, pile.getCard().getName(), pile.getCard().getCost());
            num++;
        }
    }
    ...
    public void printFormattedSupply(Supply supply) {
        ...
        for (ArrayList<SupplyPile> innerList : formattedSupply) {
            for (SupplyPile supplyPile : innerList) {
                System.out.printf("%2s) %-10s ($%s)  ", num, supplyPile.getCard().getName(), supplyPile.getCost());
    ...
    public void endGame(ArrayList<Player> players) {
        ...
        if (victor.size() > 1) {
            ...
            for (Player player : players) {
                System.out.printf("%s is a victor with %s victory points.%n", player.getName(), victoryPoints);
            }
        }
        else {
            System.out.printf("%n%s is the victor with %s victory points.%n", victor.get(0).getName(), victoryPoints);
        }
    }
}